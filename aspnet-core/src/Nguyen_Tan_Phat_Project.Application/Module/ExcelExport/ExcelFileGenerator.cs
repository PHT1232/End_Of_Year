    ...
    public class ExcelFileGenerator
    {
        public byte[] GenerateExcelFileForExportImport(List<ExportImportProductDto> list, ExportImport exportImport, Customer customer, Employee employee)
        {
            ...
            foreach (var product in list)
            {
                ...
                worksheet.Cell(rowNumber, 11).Value = product.Location;
                ...
                worksheet.Cell(rowNumber, 12).Value = product.Unit;
                ...
                worksheet.Cell(rowNumber, 13).Value = product.Quantity.ToString();
                ...
                worksheet.Cell(rowNumber, 15).Style.Border.OutsideBorder = XLBorderStyleValues.Thin;
                worksheet.Cell(rowNumber, 15).Style.Font.FontSize = 11;
                worksheet.Cell(rowNumber, 15).Style.Font.FontName = "Times New Roman";
            }
        ...
        public byte[] GenerateDeliveryExcel(List<ExportImportProductDto> list, ExportImport exportImport, Customer customer, Employee employee)
        {
            XLWorkbook workbook = new XLWorkbook("E:\\Documents\\GitHub\\End_Of_Year\\aspnet-core\\src\\Nguyen_Tan_Phat_Project.Web.Host\\wwwroot\\ExcelTemplate\\phieu xuat kho sua.xlsx");
            IXLWorksheet worksheet = workbook.Worksheet("Sheet1");
            ...
            worksheet.Cell(8, 7).Value = "Số: " + exportImport.Id.ToString();
            worksheet.Cell(8, 7).Style.Font.FontName = "Times New Roman";
            worksheet.Cell(8, 7).Style.Font.FontSize = 12;
            worksheet.Cell(8, 7).Style.Font.SetBold(true);
            ...
            foreach (var product in list)
            {
                ...
                worksheet.Cell(rowNumber, 13).Value = product.Price;
                worksheet.Cell(rowNumber, 13).Style.Border.OutsideBorder = XLBorderStyleValues.Thin;
                worksheet.Cell(rowNumber, 13).Style.Font.FontSize = 11;
                worksheet.Cell(rowNumber, 13).Style.Font.FontName = "Times New Roman";

                worksheet.Cell(rowNumber, 14).Value = product.FinalPrice;
                worksheet.Cell(rowNumber, 14).Style.Border.OutsideBorder = XLBorderStyleValues.Thin;
                worksheet.Cell(rowNumber, 14).Style.Font.FontSize = 11;
                worksheet.Cell(rowNumber, 14).Style.Font.FontName = "Times New Roman";
            }
